...
def main():
    ...
    # Parse Arguments
    parser = argparse.ArgumentParser(description='Translated MNIST Arguments')
    parser.add_argument('-d', '--dir', action="store", default='../data_trans/')
...
def convert_tfrecords(all_data, all_labels, image_dim, data_directory):
    ...
    for d in range(len(all_data)):
        ...
        writer = tf.python_io.TFRecordWriter(data_directory + "trans_mnist_" + name + ".tfrecords")
        ...
        for example_idx in tqdm(range(flags['nums'][d])):
            ...
            pixels, gt_box = generate_translated_digit(data[example_idx].reshape((28, 28)), image_dim, label, data)
...
def convert_test_png(test_data, test_labels, image_dim, data_directory):
    ...
    for example_idx in tqdm(range(flags['nums'][2])):
        ...
        label = label_np.tolist()
        pixels, gt_box = generate_translated_digit(test_data[example_idx].reshape((28, 28)), image_dim, label, test_data)
