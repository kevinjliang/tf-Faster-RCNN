...
def main():
    ...
    flags['im_dims'] = args['dim'] 
    
    process_digits(all_data, all_labels, data_directory, args)
...
def process_digits(all_data, all_labels, data_directory, args):
    ...
    for s in range(len(flags['all_names'])):
        ...
        for i in trange(flags['nums'][split]):
            ...
            img, gt_boxes = gen_nCluttered(all_data[s], all_labels[s], flags['im_dims'], 1)
...
def gen_nCluttered(data, labels, im_dims, num_digits, clutter_rate=1):
    ...
    Args:
        ...
        im_dims: width/height of output cluttered MNIST digit image
    ...
    for j in range(int(im_dims*clutter_rate)):
